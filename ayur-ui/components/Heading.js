...
const Heading = ({style,cssClass,weight,font,children}) => { 
  let [localStyle,setLocalStyle] = useState("h1"); 
  let [localFont, setLocalFont] = useState("heading");
  let [localWeight, setLocalWeight] = useState("bold");
  ...
  return( 
    <div className="relative">
      <div class={styles.config__heading}>
      ...
      <h2 className={localStyle + " font-" + localFont + " weight-" + localWeight+" " + (cssClass ?? "")}>{children}</h2>
    </div>   
  )
}
